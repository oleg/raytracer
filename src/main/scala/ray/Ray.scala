...
case class Ray(origin: Tuple, direction: Tuple) {
  ...
  def intersect(s: Sphere): List[Intersection] = transform(s.transform.inverse).selfIntersect(s)

  private def selfIntersect(s: Sphere): List[Intersection] = { //todo  return Intersections!!!
